...
const UserManagement = ({ onBack, onError, onSuccess }: UserManagementProps) => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      onError(err.response?.data?.error || 'Failed to fetch users')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSaveUser = async (data: any) => {
    try {
      ...
      onError(err.response?.data?.error || 'Failed to save user')
    } finally {
      setLoading(false)
    }
  }
