...
const Settings = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        ...
        setNationalities(natRes.data?.map((n: any) => n.name) || [
  ...
  const handleAddNationality = async () => {
    if (!newNationality.trim()) {
      setError('Please enter a nationality name')
      return
    }
    
    if (nationalities.includes(newNationality)) {
      setError('This nationality already exists')
      return
    }
    ...
    try {
      ...
      setError('Failed to add nationality')
    }
  }

  const handleDeleteNationality = (nationality: string) => {
    if (confirm(`Are you sure you want to remove ${nationality}?`)) {
  ...
  // Render Office Management content (Fee + Document templates)
  const renderOfficeManagement = () => (
    <>
  ...
  return (
    <Box>
        ...
        <TabPanel value={tabValue} index={user?.role === UserRole.SUPER_ADMIN ? 2 : 0}>
          ...
          <Box sx={{ mb: 2 }}>
            <Button
              variant="contained"
              startIcon={<AddIcon />}
              onClick={() => setNationalityDialog(true)}
            >
              Add Nationality
            </Button>
          </Box>
      ...
      <Dialog open={nationalityDialog} onClose={() => setNationalityDialog(false)} maxWidth="xs" fullWidth>
